...
public class BallGameManagerLerp : MonoBehaviour
{
    public Transform player;
    public Transform controllerLeft; 
    public Transform controllerRight;
    public ColorChangerLerp[] balls;
    ...
    private string condition = "SliderManyChoices";
    // CSV Data
    ...
    {
        currentSequenceIndex = Mathf.RoundToInt(value);
    ...
    {
        //Debug.Log("Slider moved, changing ball colors to sequence: " + sequenceIndex);

        if (sequenceIndex >= 0 && sequenceIndex < colorSequences.colorSequences[currentColorSequencesIndex].Length)
        {
            if (balls.Length == colorSequences.colorSequences[currentColorSequencesIndex][sequenceIndex].Length)
            {
                for (int i = 0; i < balls.Length; i++)
                {
                    balls[i].StartLerp(colorSequences.colorSequences[currentColorSequencesIndex][sequenceIndex][i]);
                    //Debug.Log($"Ball {balls[i].gameObject.name} color set to lerp to: {colorSequences.colorSequences[sequenceIndex][i]}");
                }
            }
            else
            {
    ...
    private void OnConfirmButtonClicked()
    {
        ...
        Debug.Log("Confirm trigger pressed");
    ...
    private void OnPlayAgainButtonClicked()
    {
        if (confirmButtonPressed && sequenceCounter < 3)
        {
            ...
            /*
            currentColorSequencesIndex++;
            if (currentColorSequencesIndex >= colorSequences.colorSequenceOrders.Length)
            {
                currentColorSequencesIndex = 0;
            }

            currentSequenceIndex = Mathf.RoundToInt(timeSlider.value);
            ChangeBallColors(currentSequenceIndex);

            Debug.Log("Current Index of the ColorOrders Array: " + currentColorSequencesIndex);
            Debug.Log("Current Color Sequence: " + colorSequences.colorSequences.ToString());
            **/
            InitializeColorSequenceForScene();
    ...
    {
        int currentColorSequenceOrder = ParticipantManager.Instance.colorSequenceOrder;
        int startIndex = (sceneNumber % 9) * 3;

        var colorSequenceOrder = colorSequences.colorSequenceOrders[currentColorSequenceOrder];
        ...
        Debug.Log($"Initializing color sequences for scene. Order index: {currentColorSequenceOrder}, Scene index: {sceneNumber}, Initial index of the Order Array: {startIndex}, " +
            $"Initial Sequence: colorSequences[{currentColorSequencesIndex}]");
